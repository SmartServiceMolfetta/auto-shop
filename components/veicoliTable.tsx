...
const VeicoliTable = ({ veicoli, className }: VeicoliTableProps) => {
    ...
    //console.log('filtri ricevuti: ', filtri);
     ...
     useEffect(() => {
        if (veicoli && filtri) {
            let _veicoliFiltrati =  veicoli.filter((veicolo) => {
                ...
                //const matchesAnno = filtri.anno && filtri.anno !== 'tutti'  ? veicolo.anno === filtri.anno : true;
                        ...
                        <TableRow>
                            ...
                            <TableHead className="whitespace-nowrap font-bold hidden sm:table-cell">
                                <div className="flex items-center justify-center gap-2">
                                    ...
                                    <span className="w-[18px] cursor-pointer" onClick={() => requestSort('proprietario')}>{sortedIcon}</span>
                                </div>
                            </TableHead>               
                        </TableRow>
                    </TableHeader> 
                    <TableBody>
                        {paginatedVeicoli && paginatedVeicoli.map((veicolo, index) => (
                            <TableRow key={index}>
                                ...
                                <TableCell className="hidden sm:table-cell text-center">{veicolo.proprietario}</TableCell>                            
                            </TableRow> 
                        ))}
                    </TableBody> 
                </Table>
